...
- [x] Rule block: `shocks = {enabled, profile, rng_seed}`; profiles off/peaceful/standard/harsh/apocalyptic/battery
- [x] Seeded PRNG event roller, deterministic & replay-safe; SHOCK_START/SHOCK_END ledger events
- [x] Effects: drought (farm output %), outage (electricity disabled), spoilage (food stock %), machine-failure wave, pandemic (labor reduction → death at severe tier), demand shift
- [x] Death accounting: balance→surplus pool, inventory→common pool, membership end, CITIZEN_DEATH event; dignity floor (never market-death)
- [x] Unit tests: PRNG determinism, each effect, death accounting

### 2 · Demographics
- [x] Births every `birth_interval_ticks` (default 400): new citizen ₡500 minted, CITIZEN_BORN event
...
- [x] Within-field split between centers/proposals (same mechanism)
...
- [x] Algorithm edit as ordinary PROPOSE/VOTE item; temporary overrides auto-expire
...
- [x] Criterion 1: ratchet closed ≤50 ticks, all seeds
- [x] Criterion 2: escalating battery ×≥2 seeds — honest breaking-tier report; below break: essentials recover ≤100 ticks post-shock; invariant exact; replay byte-exact
- [x] Criterion 3: peaceful population growth absorbed; pandemic deaths absorbed; children mature
- [x] Criterion 4: research budgets track injected signals; delegation changes outcomes; algorithm edit takes effect
- [x] Criterion 5: crisis suspends priorities visibly and ends cleanly
- [x] Criterion 6: full suite green; old saves replay byte-identical
